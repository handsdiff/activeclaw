...
describe("exec approval handlers", () => {
  ...
  it("rejects unsupported exec approval hosts", async () => {
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        message: "unsupported exec approval host: node",
      }),
    );
  });
  ...
  it("derives a command preview from the fallback command for older gateway plans", async () => {
    const { handlers, broadcasts, respond, context } = createExecApprovalFixture();
    await requestExecApproval({
      handlers,
      respond,
