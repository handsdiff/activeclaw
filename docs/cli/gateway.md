...
read_when:
  ...
  - Discovering gateways via LAN or tailnet beacons
title: "gateway"
---

# Gateway CLI

The Gateway is OpenClaw’s WebSocket server (channels, sessions, hooks).
...
`gateway status` shows the Gateway service (launchd/systemd) plus an optional RPC probe.
